...
/**
 * Parse edge list format into a graph structure
 * 
 * @param content - Raw text content of the edge list file
 * @param isDirected - Whether to treat the graph as directed (default: false for undirected)
 * @returns Parsed graph with metadata
 ...
 * 3 1
 * `;
 * const graph = parseEdgeList(content, false); // Undirected
 * // graph.nodes === 3
 * // graph.edges.length === 6 (bidirectional)
 * ```
 */
export function parseEdgeList(content: string, isDirected: boolean = false): ParsedGraph {
  logger.time('GraphParser-ParseEdgeList');
  logger.info('GraphParser', `Starting to parse edge list (${isDirected ? 'directed' : 'undirected'} graph)`);
  ...
  for (const line of lines) {
    ...
    // Add edge(s) based on graph type
    edges.push({ source, target });
    // For undirected graphs, add reverse edge (skip if self-loop)
...
/**
 * Compute out-degree for each node in the graph
 * 
 * @param graph - The graph to compute out-degrees for
 * @returns Array of out-degrees indexed by node ID
 */
export function computeOutDegree(graph: ParsedGraph): number[] {
  const outDegree = new Array(graph.nodes).fill(0);
 ...
 * @returns Adjacency list where index is source node and value is array of target nodes
 */
...
export function validateGraph(graph: ParsedGraph): void {
  ...
  // Check for isolated nodes (nodes with no outgoing edges)
  const outDegree = computeOutDegree(graph);
  const isolatedNodes = outDegree.filter((deg) => deg === 0).length;
  if (isolatedNodes > 0) {
    logger.warn(
      'GraphParser',
      `Found ${isolatedNodes} node(s) with no outgoing edges (dangling nodes)`
    );
  }
}
