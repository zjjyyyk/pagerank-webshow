  ...
  return [
    {
      ...
      description: "A small demonstration graph with 5 nodes and 6 edges. Perfect for understanding PageRank algorithm basics.",
      nodes: 5,
      edges: 6,
      source: "Custom",
      url: "",
...
export async function loadDataset(datasetName: string): Promise<ParsedGraph> {
  ...
  try {
    ...
    const graph = parseEdgeList(content, false);
    validateGraph(graph);
 ...
 * @returns Parsed graph
 */
export async function loadFromFile(file: File, isDirected: boolean = false): Promise<ParsedGraph> {
  ...
  try {
    ...
    const graph = parseEdgeList(content, isDirected);
    validateGraph(graph);
