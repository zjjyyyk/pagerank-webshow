/**
 * @fileoverview PageRank worker hook for managing Web Worker communication
 * @module hooks/usePageRankWorker
 */
...
export function usePageRankWorker() {
  const workerRef = useRef<Worker | null>(null);
  const tasksRef = useRef<Map<string, ComputeTask>>(new Map());

  useEffect(() => {
    // Create worker
    logger.info('UI', 'Initializing PageRank worker');
    workerRef.current = new Worker(
      new URL('@/workers/pagerank.worker.ts', import.meta.url),
      { type: 'module' }
    );
    ...
    return () => {
      logger.info('UI', 'Terminating PageRank worker');
